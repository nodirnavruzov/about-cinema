...
searchByFilter.action(/^\d+$/ , async (ctx) => {
  try {
    ...
    await getMoviesByGenre(ctx, genreId)
  } catch (error) {
    console.log('error', error)
...
async function getMoviesByGenre(ctx, genreId) {
  try {
    const options = {
      method: 'GET',
      headers: {
      ...
      url: `https://kinopoiskapiunofficial.tech/api/v2.2/films?genres=${genreId}&order=RATING&type=FILM&ratingFrom=5&ratingTo=10&yearFrom=1000&yearTo=3000&page=1`
    };
    const { data } = await axios(options)
    const parsedData = toHtml(data.items)
    for (let i = 0; i < parsedData.length; i++) {
      const movie = parsedData[i];
      await sendMovies(ctx, movie)
    }
  } catch (error){
    console.log(error)
    return await ctx.reply(`${ctx.i18n.t('whoops')}`)
...
async function sendMovies(ctx, movie) {
  try {
    if (movie.title && movie.title.length > 21) {
      movie.title = movie.title.slice(0, 21)
    }
    return await ctx.replyWithPhoto({url: movie.poster}, { caption: movie.html, parse_mode: 'HTML',
